...
const logAnalytics = async (linkId, req) => {
    try {
        // 1. Increment click counter on the link
        await Link_1.Link.updateOne({ _id: linkId }, { $inc: { clicks: 1 } });
        ...
        const analyticEntry = new Analytics_1.Analytics({
            linkId,
            ip,
            browser,
            device,
...
const redirectUrl = async (req, res) => {
    try {
        ...
        if (redis_1.redisClient.isOpen) {
            cachedLink = await redis_1.redisClient.get(`link:${shortCode}`);
        }
        if (cachedLink) {
        ...
        const link = await Link_1.Link.findOne({
